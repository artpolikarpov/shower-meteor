...
window.shower = window.shower || (function (window, document, undefined) {
  ...
  shower.init = function (slideSelector, progressSelector) {
    if (!shower.on) {
      ...
      shower._addEventListeners(true);

      if (shower.DOMContentLoaded) {
        shower._listeners['window DOMContentLoaded']();

        // Hack to get :target selector work after lazy initialization
        var currentSlideNumber = shower.getCurrentSlideNumber();

        if (currentSlideNumber >= 0) {
          url.replace('#');
          url.hash = shower.getSlideHash(currentSlideNumber);
        }
      }
   ...
   * @returns {Object} shower
   */
  shower.destroy = function () {
    if (shower.on) {
      shower.on = false;

      isHistoryApiSupported && history.pushState(null, null, shower.src.href);
      document.body.className = shower.src.bodyClass;
      document.body.innerHTML = shower.src.bodyHTML;
      document.body.setAttribute('style', shower.src.bodyStyle);
  ...
  shower.enterSlideMode = function (callback) {
    ...
    shower.updateActiveAndVisitedSlides(currentSlideNumber);
  ...
  shower.isSlideMode = function () {
    return /^full.*/.test(url.search.substr(1)) || shower.guest;
  };
  ...
  shower._addEventListeners = function eventListeners (action) {
    action = (action ? 'add' : 'remove') + 'EventListener';

    if (action === eventListeners.action) return;
    eventListeners.action = action;

    Object.keys(shower._listeners).forEach(function (key) {
      var _key = key.split(' ');
      elems[_key[0]][action](_key[1], shower._listeners[key], false);
    });
  }

  // Auto init if not prevented in options
  options.preventAutoInit || shower.init();

  window.addEventListener('DOMContentLoaded', shower._listeners['window DOMContentLoaded'], false);
